...
async function GptAcademicJavaScriptInit(dark, prompt, live2d, layout, tts) {
    try {
        ...
        try {
            ...
            console.warn('Welcome message initialization failed:', welcomeError);
        }

        // 安全地获取chatbot元素，添加重试机制
        let chatbotIndicator = null;
        let retryCount = 0;
        const maxRetries = 10;

        const findChatbotElement = () => {
            try {
                const app = gradioApp();
                if (app) {
                    chatbotIndicator = app.querySelector('#gpt-chatbot > div.wrap');
                }
            } catch (error) {
                console.warn('Failed to find chatbot element, attempt:', retryCount + 1, error);
            }

            if (!chatbotIndicator && retryCount < maxRetries) {
                retryCount++;
                setTimeout(findChatbotElement, 500); // 500ms后重试
            } else if (chatbotIndicator) {
                // 找到元素后设置观察器
                try {
                    var chatbotObserver = new MutationObserver(() => {
                        try {
                            ...
                            console.warn('MutationObserver callback error:', observerError);
                        }
                    });
                    chatbotObserver.observe(chatbotIndicator, { attributes: true, childList: true, subtree: true });
                } catch (observerSetupError) {
                    console.warn('Failed to setup MutationObserver:', observerSetupError);
                }
            }
        };

        // 延迟执行，确保DOM已加载
        setTimeout(findChatbotElement, 100);

        if (layout === "LEFT-RIGHT") {
            try {
                chatbotAutoHeight();
                limit_scroll_position();
            } catch (layoutError) {
        ...
        console.error('GptAcademicJavaScriptInit failed:', error);
        // 确保基本功能仍然可用
        try {
            remove_legacy_cookie();
            // 应用基本主题
